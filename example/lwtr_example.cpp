...
void record(Archive &ar, rw_task_if::write_t const& u) {
	ar & field("addr", u.addr) & field("data", u.data);
}
}
...
inline void test::main1() {
	...
	for (int i = 0; i < 3; i++) {
		...
		SCP_INFO(sc_get_current_object()->name())  << "received data : " << data;
	}
	for (int i = 0; i < 3; i++) {
		rw_task_if::addr_t addr = rand()%256;
		rw_task_if::data_t data = transactor->read(&addr);
		SCP_INFO(sc_get_current_object()->name()) << "data for address " << addr << " is " << data;
	}
	...
	for (int i = 0; i < 3; i++) {
		...
		SCP_INFO(sc_get_current_object()->name()) << "send data : " << write.data;
	}
}
...
inline void design::addr_phase() {
	while (true) {
		...
		SCP_INFO(sc_get_current_object()->name())  << "received request for memory address " << _addr;
	}
}
...
inline void design::data_phase() {
	while (true) {
		...
		if (outstandingType.front() == false) {
			SCP_INFO(sc_get_current_object()->name()) << "reading memory address " << outstandingAddresses.front() << " with value "
			...
			SCP_INFO(sc_get_current_object()->name()) << "writing memory address " << outstandingAddresses.front() << " with value " << bus_data;
