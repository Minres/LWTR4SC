...
template<typename T>
class has_record_member {
	struct Fallback { int record; };
...
template<typename T>
struct value_converter<T, typename std::enable_if<std::is_const<T>::value>::type> {
	static value to_value(T& v) { return value_converter<typename std::remove_const<T>::type>::to_value(v); }
};

template <typename T>
struct value_converter<T, typename std::enable_if<has_record_member<T>::value>::type> {
	static value to_value(T const& v) {
		access a;
		v.record(a);
		return a.get_value();
...
public:
	...
	tx_handle(tx_handle const& o):pimpl(o.pimpl) {}
...
public:
	...
	tx_handle begin_tx(const char* relation_name, tx_handle const& other_tx_h) {
		return tx_generator_base::begin_tx(value(), sc_core::sc_time_stamp(),
	...
	tx_handle begin_tx(const BEGIN& begin_attr, const char* relation_name, const tx_handle& other_tx_h) {
		auto v = lwtr::record(begin_attr);
		return tx_generator_base::begin_tx(v, sc_core::sc_time_stamp(),
				get_tx_fiber().get_tx_db()->create_relation(relation_name),
				&other_tx_h);
	}
	...
	void end_tx(tx_handle& t) {
		tx_generator_base::end_tx(t, value(), sc_core::sc_time_stamp());
	}
